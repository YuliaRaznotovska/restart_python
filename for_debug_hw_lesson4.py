"""
симулятор каси магазину
код, наведений нижче, приймає від покупця наступну інформацію про закупівлю 2-х товарів
- назва
- кількість (ціле число)
- ціна за одиницю
на підставі отриманих даних формується чек
всі ціни та вартість мають бути виведені в форматі з копійками, кількість - цілі числа
програма розрахована на використання на території України
"""
import textwrap
from datetime import datetime
from decimal import Decimal
# goods 1 section
item_1_title = textwrap.shorten(input('Введіть назву першого товару: ').ljust(20, '.'), width=20, placeholder='...')
item_1_quantity = Decimal(input('Введіть бажаєму кількість першого товару : ')).quantize(Decimal('1'))
# item_1_quantity = int(Decimal(input('Введіть бажану кількість першого товару: ')))  # цілі числа
item_1_zina = Decimal(input('Введіть ціну першого товару: ')).quantize(Decimal('0.01'))

# goods 2 section
item_2_title = textwrap.shorten(input('Введіть назву другого товару: ').ljust(20, '.'), width=20, placeholder='...')
item_2_quantity = Decimal(input('Введіть бажаєму кількість другого товару : ')).quantize(Decimal('1'))
# item_2_quantity = int(Decimal(input('Введіть бажану кількість другого товару: ')))
item_2_zina = Decimal(input('Введіть ціну другого товару: ')).quantize(Decimal('0.01'))

total_quantity = item_1_quantity + item_2_quantity

item_1_total_cost = Decimal(int(item_1_quantity)).quantize(Decimal('1.0')) * Decimal(float(item_1_zina)).quantize(Decimal('0.01'))
item_1_total_cost_right_format = item_1_total_cost.quantize(Decimal('0.01'))

item_2_total_cost = Decimal(int(item_2_quantity)).quantize(Decimal('1.0')) * Decimal(float(item_2_zina)).quantize(Decimal('0.01'))
item_2_total_cost_right_format = item_2_total_cost.quantize(Decimal('0.01'))

total_cost_final = item_1_total_cost_right_format + item_2_total_cost_right_format

printing_template = '{}\t\t\t\t\t{}\t\t\t\t{}\t\t\t{}'

# printing receipt
print('\n\n\n')
print('фіскальний чек'.capitalize().center(80, '~'))
print('магазин "все для дому"'.upper().center(80))
print(f'Товар\t\t\t\t\t\t\t\t\tкількість\t\tціна\t\tвартість')
print(printing_template.format(item_1_title, item_1_quantity, item_1_zina, item_1_total_cost_right_format))
print(printing_template.format(item_2_title, item_2_quantity, item_2_zina, item_2_total_cost_right_format))
print('~' * 80)
print(printing_template.format(
    'ВСЬОГО'.ljust(20),
    total_quantity,
    'x',
    total_cost_final
    )
)
print(datetime.now().strftime('%d-%m-%Y %H:%M:%S').rjust(80))
print('\n\n')
